...
module.exports = {
    ...
    getRetirementQuote: (req, res) => {
        const retirementQuote = ["Retirement is more fun than work, and you never have to make excuses. -Unknown",
    ...
    createRetirement: (req, res) => {
        ...
        res.status(200).json({ success: true, message: 'success',results:results });
        },

    deleteRetirementData: (req, res) => {
            const { index } = req.params
            // let index = results(elem => elem === req.params.index)
            if (index !== -1) {
